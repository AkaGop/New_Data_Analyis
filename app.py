...
if uploaded_file:
    ...
    st.markdown("---")
    # Dashboard without Job Status
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Lot ID", str(summary['lot_id']))
    c2.metric("Total Panels", int(summary['panel_count']))
    c3.metric("Job Duration (sec)", f"{summary['total_duration_sec']:.2f}")
    c4.metric("Avg Cycle Time (sec)", f"{summary['avg_cycle_time_sec']:.2f}")

    with st.expander("Show Exploratory Data Analysis (EDA)"):
        st.subheader("Event Frequency")
        st.bar_chart(eda_results.get('event_counts', pd.Series(dtype='int64')))

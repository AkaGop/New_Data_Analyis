...
def _parse_s6f11_report(full_text: str) -> dict:
    ...
    if rptid in RPTID_MAP:
        data['RPTID'] = rptid
        try:
            rptid_index = [i for i, v in enumerate(re.findall(r"'[^']*'|\d+", full_text)) if v == str(rptid)][0]
            payload = [s if s else i for s, i in re.findall(r"<(?:A|U\d)\s\[\d+\]\s(?:'([^']*)'|(\d+))>", full_text)][rptid_index + 1:]

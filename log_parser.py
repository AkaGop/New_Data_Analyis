...
def _parse_s6f11_report(full_text: str) -> dict:
    ...
    if rptid in RPTID_MAP:
        ...
        # A[16] is a timestamp, but we check for any long digit string for robustness
        data_payload_filtered = [val for val in payload if not (len(val) > 10 and val.isdigit())]
